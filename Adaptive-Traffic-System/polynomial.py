...
# Fit linear regression models for EW and NS directions
model_ew = LinearRegression()
model_ns = LinearRegression()

model_ew.fit(X_train_poly, y_ew_train)
model_ns.fit(X_train_poly, y_ns_train)

# Predict green times for EW and NS directions
y_ew_pred = model_ew.predict(X_test_poly)
y_ns_pred = model_ns.predict(X_test_poly)
...
new_data = pd.DataFrame({
    'cycle_length': [120],
    'lane1_num_vehicles': [46],
    'lane2_num_vehicles': [25],
    'lane3_num_vehicles': [15],
    'lane4_num_vehicles': [57],
    'ew_waiting_times': [20],
    'time_of_day': [10],
    'flow_ratios': [1.1]
})
...
predicted_ew_green_time = model_ew.predict(new_data_poly)
predicted_ns_green_time = model_ns.predict(new_data_poly)
